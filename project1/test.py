...
TODO: Fill out the docstring comment with usage.
"""
...
def wait_for_test_process(proc, timeout):
    ...
    while proc.returncode is None and time.time() < endtime:
        time.sleep(0.1)
        err_chunks.append(os.read(proc.stderr.fileno(), 4096).decode('utf-8'))
        proc.poll()
    ...
    chunk = True
    while chunk:
        chunk = os.read(proc.stderr.fileno(), 4096).decode('utf-8')
        if chunk:
...
def test_project1(binary):
            ...
            for lineIndex in range(0, len(lines)):
                ...
                if match:
                    ...
                    if passed:
                        ...
                        num_failed += 1
                    # NOTE: Added 'or' to print failed tests.
    ...
    # No need for this when we're printing failed tests above.
    if QUIET and False:
